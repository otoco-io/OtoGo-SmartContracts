...
contract('Stake Tests', async (accounts) => {
  ...
  it('Distribute tokens for accounts to stake', async function() {
    ...
    await this.token.transfer(accounts[4], web3.utils.toWei('1000000','ether'))
    await this.token.transfer(accounts[5], web3.utils.toWei('1000000','ether'))
    await this.token.transfer(accounts[6], web3.utils.toWei('1000000','ether'))
    ...
    await this.token.approve(this.pool.address, web3.utils.toWei('1000000','ether'), {from:accounts[4]});
    await this.token.approve(this.pool.address, web3.utils.toWei('1000000','ether'), {from:accounts[5]});
    await this.token.approve(this.pool.address, web3.utils.toWei('1000000','ether'), {from:accounts[6]});
    let balance2 = await this.token.balanceOf(accounts[2]);
    let balance3 = await this.token.balanceOf(accounts[3]);
    let balance4 = await this.token.balanceOf(accounts[4]);
    let balance5 = await this.token.balanceOf(accounts[5]);
    let balance6 = await this.token.balanceOf(accounts[6]);
    // Verify balances
    ...
    expect(balance4.toString()).to.equal(web3.utils.toWei('1000000','ether'));
    expect(balance5.toString()).to.equal(web3.utils.toWei('1000000','ether'));
    expect(balance6.toString()).to.equal(web3.utils.toWei('1000000','ether'));
  });

  it ('Stake launch pool', async function () {
    await this.pool.stake(this.token.address, web3.utils.toWei('400000'), {from:accounts[2]});
    await this.pool.stake(this.token.address, web3.utils.toWei('400000'), {from:accounts[3]});
    await this.pool.stake(this.token.address, web3.utils.toWei('400000'), {from:accounts[4]});
    await this.pool.stake(this.token.address, web3.utils.toWei('400000'), {from:accounts[5]});
    await this.pool.stake(this.token.address, web3.utils.toWei('400000'), {from:accounts[6]});
